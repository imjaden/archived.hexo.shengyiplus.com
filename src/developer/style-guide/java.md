...
- 不允许任何魔法值（即未经预先定义的常量）直接出现在代码中。反例：
  ...
  说明：上面的”Id#taobao_”就是魔法值，需要先定义再使用：正例：
...
- POJO类必须写toString方法。
    - 使用IDE中的工具：source> generate toString时，如果继承了另一个 POJO 类，注意在前面加一下 super.toString。 
...
- 循环体内，字符串的连接方法，使用StringBuilder的append方法进行扩展。 
